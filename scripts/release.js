...
const paths = {
    ...
    docsDir: path.join(rootDir, 'docs', 'docs'),
    packageJson: path.join(rootDir, 'package.json'),
    releasedDocsDir: path.join(rootDir, 'docs', 'versioned_docs', 'version-stable')
};
...
function ensureTagDoesNotExist(newVersion) {
...
function snapshotReleasedDocs() {
    if (!fs.existsSync(paths.docsDir)) {
        throw new Error(`Docs directory does not exist: ${paths.docsDir}`);
    }

    fs.rmSync(paths.releasedDocsDir, {
        force: true,
        recursive: true
    });
    fs.cpSync(paths.docsDir, paths.releasedDocsDir, {
        recursive: true
    });
}
...
async function main() {
    ...
    console.log('Updating the released docs snapshot...');
    snapshotReleasedDocs();
