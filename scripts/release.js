...
function ensureVersionDoesNotExist(newVersion) {
    ...
    const versionedSidebarFile = path.join(paths.versionedSidebarsDir, `version-${newVersion}-sidebars.json`);
    if (fs.existsSync(versionedSidebarFile)) {
        throw new Error(`Docs version sidebar already exists: ${versionedSidebarFile}`);
    }
}
...
function snapshotVersionedDocs(previousVersion, newVersion) {
    ...
    fs.cpSync(paths.docsDir, path.join(paths.versionedDocsDir, `version-${newVersion}`), {
        recursive: true
    });
